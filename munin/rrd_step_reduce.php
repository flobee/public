...
if (isset($_SERVER['argv'][1]) && file_exists($_SERVER['argv'][1])) {
    ...
    exit('rrd xml file not found');
}

if (isset($_SERVER['argv'][2])) {
    if( ($newInterval = (int) $_SERVER['argv'][2]) == 0) {
        exit('new interval invalid');
    }
}
else {
    exit('new interval not set');
}
...
//
//$oXML = new SimpleXMLElement(file_get_contents($file));
//echo $oXML->step;
//$oXML->step = 1200;
//$oXML->saveXML(dirname($file) . '/temp.' . $newInterval . '.xml');
