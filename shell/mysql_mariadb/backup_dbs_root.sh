...
# myloader -u ${MYSQL_USER} -p ${MYSQL_PASS} -h ${MYSQL_HOST} --port=${MYSQL_PORT} ${MYSQL_SOCK} --overwrite-tables --threads 4 --database ${DBNAME} --verbose 1 --directory "${OUTDIR}/${DBNAME}"


# backupd each database in a seperated directory
#
...
# depending on your setup, eg. synology mariadb5 and 10 need a socket or you get only v5 even if the port if different
...
echo "job starts in 3 sec... to abort press CNTR + C";
sleep 3;
